...
{
	$_SESSION["author"]	=	urldecode($_GET["p_author"]);
}

if(isset($_SESION["author"]) AND isset($_GET["p_author"]))
{
	if($_SESSION["author"] != urldecode($_GET["p_author"]))
	{
	session_destroy();
	$_SESSION["author"]	=	urldecode($_GET["p_author"]);
	}
...
<?php
$tableName		=	"product";
$query 			= 	"SELECT COUNT(*) as num FROM $tableName WHERE p_author='".$_SESSION["author"]."' ORDER BY id DESC ";
$query1a 		= 	"SELECT * FROM $tableName WHERE p_author='".$_SESSION["author"]."' ORDER BY id DESC ";
$targetpage		=	"search-author.php";
$limit 			= 	"12"; 
